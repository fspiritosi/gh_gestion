...
//   console.log(id)
//   //   Cargar documentos del contrato al iniciar
//   useEffect(() => {
//     fetchDocuments()
//   }, [id])
...
//   //   Función para obtener documentos de Supabase
//   const fetchDocuments = async () => {
//     try {
//       setIsLoading(true)

//       // Consultar la tabla de documentos para obtener los metadatos
...
//       // Para cada documento, obtener la URL firmada del storage
//       const documentsWithUrls = await Promise.all(
//         documentMetadata.map(async (doc) => {
//           const { data: urlData } = await supabase.storage.from("contract-documents").createSignedUrl(doc.path, 60 * 60) // URL válida por 1 hora
...
//   // Función para procesar archivos seleccionados (sin subirlos)
...
//   // Función para subir los archivos seleccionados
...
//         // Crear un path único para el archivo en Supabase Storage
//         const timestamp = Date.now()
//         const filePath = `${contract?.id}/${timestamp}-${file.name}`
...
//         // Guardar los metadatos del documento en la tabla 'documents'
//         console.log
...
//         // Eliminar el registro de la tabla 'documents'
...
//     try {
//       // Si la URL ya está disponible y es válida, usarla directamente
//       if (doc.url) {
//         window.open(doc.url, "_blank")
//         return
//       }

//       // Si no hay URL o expiró, obtener una nueva URL firmada
//       const { data, error } = await supabase.storage.from("contract-documents").createSignedUrl(doc.path, 60 * 60) // URL válida por 1 hora
...
export default function contractDocuments({ id }: DocumentManagementProps) {
  ...
  // Función para crear un nombre de carpeta seguro (sin caracteres especiales)
  ...
  const uploadSelectedFiles = async () => {
    ...
    try {
      ...
      // Obtener el nombre del cliente y el título del contrato
      ...
      // Crear nombres de carpetas seguros
      ...
      for (const file of selectedFiles) {
        ...
        // Crear un path único para el archivo en Supabase Storage
        const timestamp = Date.now();
        // Estructura: nombre_cliente/titulo_contrato/timestamp-nombre_archivo
