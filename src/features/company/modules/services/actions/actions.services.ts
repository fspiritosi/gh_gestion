...
export const ServiceUpdate = async (values: Service, modified_editing_service_id: string, resetForm: () => void, router: any) => {
  const data = JSON.stringify(values);
  try {
...
export const handleDeactivateService = async (editingService: Service & { is_active: boolean }, modified_editing_service_id: string, setFilteredServices: any, router: any) => {
  if (editingService) {
    try {
