...
export async function fetchContact(contactId: string) {
  ...
  try {
    const { data, error } = await supabase.from('contacts').select('*').eq('id', contactId);
...
export async function deactivateContact(values: any, contactId: string, companyId: string) {
  const supabase = supabaseServer();
