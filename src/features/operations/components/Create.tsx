...
export default function Create() {
  ...
  const saveDate = async (date: string) => {
    try {
      ...
      const { data: newDailyReport } = await dailyReportResponse.json()
      const dailyReportId = newDailyReport[0]

      // Fetch all reports for the company to pass to DailyReport
      const allReportsResponse = await fetch(`/api/daily-report/company/${company_id}`)
      const allReportsData = await allReportsResponse.json()

      // Prepare the report data structure expected by DailyReport
      const reportDataObj = {
        id: dailyReportId,
        date: date,
        status: true,
    ...
    <div>
      ...
      <Dialog open={openModal} onOpenChange={setOpenModal}>
        <DialogContent className="max-w-[30vw] h-[40vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>Parte Diario</DialogTitle>
            <DialogDescription>Seleccione la Fecha del nuevo parte diario</DialogDescription>
          </DialogHeader>
          <div className="flex-grow overflow-auto">
          ...
          <DialogFooter>
            ...
            <Button disabled={!date} onClick={handleSubmit(onSubmit)}>
              Crear
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={openModal1} onOpenChange={setOpenModal1}>
        <DialogContent className="max-w-[95vw] h-[90vh] flex flex-col">
          ...
          <div className="flex-grow overflow-auto">
            {reportData && <DailyReport reportData={reportData} allReport={allReports} />}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={handleCloseModal1}>
              Cerrar
            </Button>
          </DialogFooter>
        </DialogContent>
